...
const LostItemsPage = () => {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await lostItemApi.getAllLostItems(page - 1, 12, { query: search, category });
      const pagedData = res?.data || res;
      const contentList = pagedData?.content || (Array.isArray(pagedData) ? pagedData : []);
      
      // Filter locally if query/category params were passed
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1200, mx: 'auto' }}>
      <Typography variant="h4" gutterBottom sx={{ fontWeight: 'bold' }}>Lost Items</Typography>
            ...
            <Grid item xs={12}>
              <Paper sx={{ p: 6, textAlign: 'center', borderRadius: 3 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  If you lost something, click "Report Lost Item" to create a post!
                </Typography>
              </Paper>
            </Grid>
          )}
